...
s3_client = boto3.client('s3', aws_access_key_id=os.environ.get('AWS_ACCESS_KEY_ID'), aws_secret_access_key=os.environ.get('AWS_SECRET_ACCESS_KEY'), region_name=os.environ.get('AWS_REGION', 'eu-north-1')) if os.environ.get('AWS_ACCESS_KEY_ID') else None
...
def initialize_database():
    conn = get_db_connection(); cur = conn.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS institutions (school_id SERIAL PRIMARY KEY, school_name VARCHAR(150) NOT NULL UNIQUE, subscription_expiry_date DATE NOT NULL, address VARCHAR(255) DEFAULT 'Ghana', phone VARCHAR(50) DEFAULT '0000000000', primary_color VARCHAR(20) DEFAULT '#0f4c81', logo_key VARCHAR(255))")
    cur.execute("CREATE TABLE IF NOT EXISTS students (student_id SERIAL PRIMARY KEY, school_id INTEGER REFERENCES institutions(school_id) ON DELETE CASCADE, first_name VARCHAR(100) NOT NULL, last_name VARCHAR(100) NOT NULL, guardian_name VARCHAR(100) NOT NULL, guardian_contact VARCHAR(20) NOT NULL, enrollment_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP, boarding_status VARCHAR(20) DEFAULT 'Day', house VARCHAR(100) DEFAULT 'Unassigned', current_class VARCHAR(100) DEFAULT 'Unassigned', transport_route VARCHAR(100) DEFAULT 'None', photo_key VARCHAR(255))")
    cur.execute("CREATE TABLE IF NOT EXISTS system_users (user_id SERIAL PRIMARY KEY, school_id INTEGER REFERENCES institutions(school_id) ON DELETE CASCADE, email VARCHAR(100) UNIQUE NOT NULL, password_hash VARCHAR(255) NOT NULL, role VARCHAR(20) NOT NULL, linked_student_id INTEGER REFERENCES students(student_id) ON DELETE CASCADE, phone VARCHAR(20), subject VARCHAR(100), base_salary DECIMAL(10,2) DEFAULT 0.00)")
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS fees (fee_id SERIAL PRIMARY KEY, school_id INTEGER REFERENCES institutions(school_id) ON DELETE CASCADE, student_id INTEGER REFERENCES students(student_id) ON DELETE CASCADE, description VARCHAR(255) NOT NULL, amount_due DECIMAL(10, 2) NOT NULL, date_issued TIMESTAMP DEFAULT CURRENT_TIMESTAMP, fee_category VARCHAR(50) DEFAULT 'General', academic_year VARCHAR(9) DEFAULT 'Unknown', term VARCHAR(20) DEFAULT 'Unknown')")
    ...
    if not cur.fetchone(): cur.execute("INSERT INTO system_users (email, password_hash, role) VALUES (%s, %s, %s)", ('superadmin@engine.com', generate_password_hash('ceo123'), 'superadmin'))
...
def automated_weekly_backup():
    ...
    for school in schools:
        ...
        cur.execute("SELECT * FROM subjects WHERE school_id = %s", (school['school_id'],)); backup['data']['subjects'] = cur.fetchall()
        cur.execute("SELECT * FROM students WHERE school_id = %s", (school['school_id'],)); backup['data']['students'] = cur.fetchall()
        cur.execute("SELECT * FROM fees WHERE school_id = %s", (school['school_id'],)); backup['data']['fees'] = cur.fetchall()
        cur.execute("SELECT p.* FROM payments p JOIN fees f ON p.fee_id = f.fee_id WHERE f.school_id = %s", (school['school_id'],)); backup['data']['payments'] = cur.fetchall()
        cur.execute("SELECT * FROM grades WHERE school_id = %s", (school['school_id'],)); backup['data']['grades'] = cur.fetchall()
        try: s3_client.put_object(Bucket=AWS_BUCKET_NAME, Key=f"Automated_Backups/{school['school_name'].replace(' ', '_')}/Backup_{datetime.now().strftime('%Y%m%d')}.json", Body=json.dumps(backup, default=custom_json_serializer))
...
def login():
    try:
        ...
        if u and check_password_hash(u['password_hash'], data.get('password')):
            login_user(User(u['user_id'], u['email'], u['role'], u['linked_student_id'], u['school_id']))
            return jsonify({"message": f"Logged in as {u['role']}."})
...
def verify_password():
    ...
    u = cur.fetchone(); cur.close(); conn.close()
    if u and check_password_hash(u['password_hash'], d.get('password')): return jsonify({"message": "Vault unlocked."}), 200
...
def change_password():
    ...
    try:
        cur.execute("SELECT password_hash FROM system_users WHERE user_id = %s", (current_user.id,))
        if not check_password_hash(cur.fetchone()['password_hash'], d.get('current_password')): return jsonify({"error": "Incorrect current password."}), 403
        cur.execute("UPDATE system_users SET password_hash = %s WHERE user_id = %s", (generate_password_hash(d.get('new_password')), current_user.id))
...
def admin_reset_password():
    ...
    d = request.get_json(); conn = get_db_connection(); cur = conn.cursor()
    try:
        cur.execute("UPDATE system_users SET password_hash = %s WHERE email = %s AND school_id = %s RETURNING user_id", (generate_password_hash(d.get('new_password')), d.get('target_email'), current_user.school_id))
        if not cur.fetchone(): return jsonify({"error": "User not found."}), 404
        ...
        conn.commit(); return jsonify({"message": "Password reset successfully!"}), 200
    except Exception as e: conn.rollback(); return jsonify({"error": str(e)}), 500
    finally: cur.close(); conn.close()
...
def superadmin_reset_admin():
    ...
    d = request.get_json(); conn = get_db_connection(); cur = conn.cursor()
    try:
        cur.execute("UPDATE system_users SET password_hash = %s WHERE email = %s RETURNING user_id", (generate_password_hash(d.get('new_password')), d.get('admin_email')))
        if not cur.fetchone(): return jsonify({"error": "Admin not found."}), 404
        conn.commit(); return jsonify({"message": "Global Override successful!"}), 200
    except Exception as e: conn.rollback(); return jsonify({"error": str(e)}), 500
    finally: cur.close(); conn.close()
...
def superadmin_update_credentials():
    ...
    d = request.get_json(); conn = get_db_connection(); cur = conn.cursor()
    try:
        if d.get('new_email'): cur.execute("UPDATE system_users SET email = %s WHERE user_id = %s", (d.get('new_email'), current_user.id))
        if d.get('new_password'): cur.execute("UPDATE system_users SET password_hash = %s WHERE user_id = %s", (generate_password_hash(d.get('new_password')), current_user.id))
        conn.commit(); return jsonify({"message": "Master credentials updated!"}), 200
    except Exception as e: conn.rollback(); return jsonify({"error": str(e)}), 500
    finally: cur.close(); conn.close()
...
def onboard_school():
    ...
    try:
        ...
        new_id = cur.fetchone()['school_id']
        cur.execute("INSERT INTO system_users (school_id, email, password_hash, role) VALUES (%s, %s, %s, 'admin')", (new_id, data.get('admin_email'), generate_password_hash(data.get('admin_password'))))
        conn.commit(); return jsonify({"message": f"Onboarded {data.get('school_name')}!"}), 201
    except Exception as e: conn.rollback(); return jsonify({"error": str(e)}), 409
    finally: cur.close(); conn.close()
...
def update_branding(school_id):
    ...
    conn.commit(); cur.close(); conn.close(); return jsonify({"message": "Branding updated successfully!"})

# --- ALL ADMIN OPERATIONAL ROUTES ---
...
def manage_students():
    ...
    if request.method == 'POST':
        d = request.get_json(); photo_b64 = d.get('photo_b64'); photo_key = None
        ...
        cur.execute("INSERT INTO students (school_id, first_name, last_name, current_class, transport_route, guardian_name, guardian_contact, boarding_status, house, photo_key) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING student_id", 
                    (current_user.school_id, d.get('first_name'), d.get('last_name'), d.get('current_class', 'Unassigned'), d.get('transport_route', 'None'), d.get('guardian_name'), d.get('guardian_contact'), d.get('boarding_status', 'Day'), d.get('house', 'Unassigned'), photo_key))
...
def update_delete_student(student_id):
    ...
    if request.method == 'PUT':
        d = request.get_json()
        try:
            cur.execute("UPDATE students SET first_name=%s, last_name=%s, current_class=%s, transport_route=%s, boarding_status=%s, house=%s, guardian_contact=%s WHERE student_id=%s AND school_id=%s", 
            ...
            conn.commit(); return jsonify({"message": f"Student ID {student_id} updated."}), 200
        except Exception as e: conn.rollback(); return jsonify({"error": str(e)}), 500
        finally: cur.close(); conn.close()
        ...
        try:
            ...
            return jsonify({"message": f"Student ID {student_id} deleted."}), 200
        except Exception as e: conn.rollback(); return jsonify({"error": str(e)}), 500
        finally: cur.close(); conn.close()
...
def bulk_enroll():
    ...
    try:
        ...
        for row in csv_input:
            if len(row) >= 2: 
                cur.execute("INSERT INTO students (school_id, first_name, last_name, current_class, guardian_name, guardian_contact, boarding_status, house) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
                            (current_user.school_id, row[0].strip()[:100], row[1].strip()[:100], row[2].strip()[:100] if len(row)>2 else 'Unassigned', row[3].strip()[:100] if len(row)>3 else 'N/A', row[4].strip()[:20] if len(row)>4 else '', row[5].strip()[:20] if len(row)>5 else 'Day', row[6].strip()[:100] if len(row)>6 else 'Unassigned'))
                count += 1
        ...
        conn.commit(); return jsonify({"message": f"Enrolled {count} students."}), 201
...
def promote_students():
    if current_user.role != 'admin': return jsonify({"error": "Admin only."}), 403
    d = request.get_json()
    if not d.get('from_class') or not d.get('to_class'): return jsonify({"error": "Provide both classes."}), 400
    conn = get_db_connection(); cur = conn.cursor()
    try:
        cur.execute("UPDATE students SET current_class = %s WHERE current_class = %s AND school_id = %s RETURNING student_id", (d.get('to_class'), d.get('from_class'), current_user.school_id))
        promoted = cur.fetchall()
        cur.execute("INSERT INTO audit_logs (school_id, user_email, action, target) VALUES (%s, %s, %s, %s)", (current_user.school_id, current_user.email, 'Promoted Cohort', f"Moved {len(promoted)} from {d.get('from_class')} to {d.get('to_class')}"))
        conn.commit(); return jsonify({"message": f"Promoted {len(promoted)} students."}), 200
    except Exception as e: conn.rollback(); return jsonify({"error": str(e)}), 500
    finally: cur.close(); conn.close()
...
def bulk_bill():
    ...
    d = request.get_json(); conn = get_db_connection(); cur = conn.cursor()
    try:
        cur.execute("INSERT INTO fees (school_id, student_id, fee_category, description, amount_due, academic_year, term) SELECT school_id, student_id, %s, %s, %s, %s, %s FROM students WHERE current_class = %s AND school_id = %s RETURNING fee_id", 
                    (d.get('fee_category'), d.get('description'), float(d.get('amount_due') or 0), d.get('academic_year'), d.get('term'), d.get('target_class'), current_user.school_id))
        billed = cur.fetchall()
...
def reverse_bill(fee_id):
    ...
    try:
        ...
        res = cur.fetchone()
        if not res: return jsonify({"error": "Bill not found."}), 404
        cur.execute("INSERT INTO audit_logs (school_id, user_email, action, target) VALUES (%s, %s, %s, %s)", (current_user.school_id, current_user.email, 'Reversed Bill', f"Fee ID {fee_id} (GHS {res['amount_due']})"))
...
def log_payment():
    ...
    try:
        ...
        if not cur.fetchone(): return jsonify({"error": "Fee ID does not exist!"}), 404
...
@login_required
def momo_initialize():
    d = request.get_json(); fee_id = int(d.get('fee_id')); amt = float(d.get('amount')); phone = str(d.get('phone') or '').strip()
    conn = get_db_connection(); cur = conn.cursor()
    ...
    if PAYSTACK_SECRET_KEY:
        try:
            res = requests.post("https://api.paystack.co/transaction/initialize", json={"email": current_user.email, "amount": int(amt * 100), "currency": "GHS", "channels": ["mobile_money", "card"], "metadata": {"fee_id": fee_id, "student_id": fee['student_id'], "phone": phone}}, headers={"Authorization": f"Bearer {PAYSTACK_SECRET_KEY}", "Content-Type": "application/json"}).json()
            cur.close(); conn.close()
            if res.get('status'): return jsonify({"status": "paystack_redirect", "auth_url": res['data']['authorization_url']})
            return jsonify({"error": res.get('message', 'Paystack rejected transaction.')}), 400
        except Exception as e: cur.close(); conn.close(); return jsonify({"error": f"Gateway error: {str(e)}"}), 500
    else:
        try:
            cur.execute("INSERT INTO payments (fee_id, amount_paid, payment_method) VALUES (%s, %s, %s)", (fee_id, amt, f"MTN MoMo Sandbox ({phone})"))
            cur.execute("INSERT INTO audit_logs (school_id, user_email, action, target) VALUES (%s, %s, %s, %s)", (current_user.school_id, current_user.email, 'MoMo Payment (Sandbox)', f"Paid GHS {amt} for Fee ID {fee_id}"))
            conn.commit(); cur.close(); conn.close(); return jsonify({"status": "cleared", "message": f"[SANDBOX MODE] Simulated payment approved for {phone}!"}), 200
...
def get_analytics():
    ...
    td = float(cur.fetchone()['total_due'])
    ...
    perf = cur.fetchall(); cur.close(); conn.close()
    return jsonify({"financials": {"due": td, "paid": tp, "outstanding": td - tp, "expenses": te, "net_margin": tp - te}, "performance": perf})
...
def add_grade():
    d = request.get_json(); conn = get_db_connection(); cur = conn.cursor()
    try:
        c_score = int(str(d.get('class_score') or '0').strip()); e_score = int(str(d.get('exam_score') or '0').strip())
        stu_id = int(str(d.get('student_id') or '0').strip()); t_score = c_score + e_score; waec = get_waec_grade(t_score)
...
def dashboard():
    ...
    # THE FULL UNTRUNCATED FRONTEND HTML
    html_template = """
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <style>
            ...
            .sidebar-header { margin-bottom: 25px; padding-bottom: 15px; border-bottom: 1px solid rgba(255,255,255,0.15); display: flex; align-items: center; gap: 12px; }
            .sidebar h2 { margin: 0; font-size: 1.1rem; line-height: 1.3; font-weight: 600;}
            .sidebar button { background: transparent; color: rgba(255,255,255,0.85); border: none; padding: 10px 14px; width: 100%; text-align: left; margin-bottom: 4px; border-radius: 8px; cursor: pointer; transition: all 0.2s ease; font-size: 0.95rem; font-weight: 500;}
            .sidebar button:hover { background: rgba(255,255,255,0.15); color: white; padding-left: 18px;}
            .main-content { margin-left: 260px; flex: 1; padding: 35px; box-sizing: border-box; min-height: 100vh; }
            .card { background: white; padding: 28px; border-radius: 16px; box-shadow: 0 10px 30px rgba(0,0,0,0.04); border: 1px solid #eaeaea; margin-bottom: 25px;}
            h3 { margin-top: 0; color: var(--primary); border-bottom: 2px solid #f0f0f0; padding-bottom: 10px; margin-bottom: 20px;}
            input, select, textarea { width: 100%; padding: 12px; margin-bottom: 15px; border: 1px solid #ced4da; border-radius: 8px; box-sizing: border-box; font-family: inherit;}
            input:focus, select:focus, textarea:focus { outline: none; border-color: var(--primary); box-shadow: 0 0 0 3px rgba(0,0,0,0.05); }
            .btn { background: var(--primary); color: white; border: none; padding: 12px 20px; border-radius: 8px; cursor: pointer; font-weight: bold; width: 100%; margin-bottom: 10px; transition: all 0.2s ease; box-shadow: 0 4px 6px rgba(0,0,0,0.1);}
            ...
            .metric-box { padding: 20px; border-radius: 12px; color: white; text-align: center; font-size: 1.1rem; font-weight: bold;}
            #toast { display: none; position: fixed; bottom: 30px; right: 30px; padding: 15px 25px; color: white; background: var(--accent); border-radius: 8px; z-index: 1000; font-weight: bold; box-shadow: 0 10px 30px rgba(0,0,0,0.2);}
    ...
    <body>
        ...
        <div class="sidebar">
            <div class="sidebar-header">
                {% if current_user.is_authenticated and current_user.school_id %}
                    <img src="/api/logo/{{ current_user.school_id }}" style="width: 42px; height: 42px; object-fit: contain; background: white; border-radius: 8px; padding: 2px;">
                {% else %}
            ...
            {% if current_user.is_authenticated %}
                <div style="font-size: 0.8rem; color: rgba(255,255,255,0.6); margin-bottom: 20px; text-transform: uppercase; font-weight: bold;">Role: {{ current_user.role }}</div>
                ...
                {% if current_user.role == 'superadmin' %}
                    ...
                    <button class="btn-success" onclick="sendAction('/api/setup_db', {}, true)" style="color:white; margin-top:20px;">🔄 Sync Engine</button>
            ...
            {% else %}
                <button class="btn-success" style="color:white;" onclick="sendAction('/api/setup_db', {}, true)">Sync System</button>
            {% endif %}
        </div>
        ...
        <main class="main-content">
            ...
            {% elif current_user.role in ['admin', 'teacher'] %}
                    ...
                    <div class="grid-2">
                        <div>
                            <div style="background: #f8f9fa; padding: 20px; border-radius: 12px; margin-bottom: 20px; border: 1px solid #eee;">
                                <h4 style="margin-top:0; color: var(--primary);">1. Issue Segmented Bill</h4>
                    ...
                    <div>
                        <div style="background: white; padding: 20px; border-radius: 12px; margin-bottom: 20px; border: 1px solid #eee; box-shadow: 0 4px 6px rgba(0,0,0,0.02);">
                            ...
                            <div style="margin-top: 15px;" class="grid-2">
                                <input type="number" id="trStuId" placeholder="ID for Transcript">
                                <button class="btn btn-primary" onclick="generateTranscript()">📜 Get Transcript</button>
                            </div>
                        </div>
                        <div style="background: #fff3cd; padding: 20px; border-radius: 12px; border: 1px solid var(--warning); margin-bottom: 20px;">
                ...
                <div id="transport-section" class="card grid-2 admin-section hidden" style="border-top: 5px solid #fd7e14;">
                    ...
                    <div>
                        <div style="background: #e9ecef; padding: 20px; border-radius: 12px; border: 1px solid #ccc; height:100%; box-sizing:border-box;">
                            <h3 style="margin-top:0; color:#333;">Active Transport Routes</h3>
                ...
                <div id="sickbay-section" class="card grid-2 admin-section hidden" style="border-top: 5px solid #d63384;">
                    <div>
                        <div style="background: #f8f9fa; padding: 20px; border-radius: 12px; border: 1px solid #eee;">
                            ...
                            <button class="btn" style="background:#d63384;" onclick="sendAction('/api/sickbay', {student_id: document.getElementById('sbStuId').value, symptoms: document.getElementById('sbSymp').value, treatment: document.getElementById('sbTreat').value})">Log Medical Record</button>
                        </div>
                    </div>
                    <div>
                        <div style="background: #e9ecef; padding: 20px; border-radius: 12px; border: 1px solid #ccc; height:100%; box-sizing:border-box;">
                            <h3 style="margin-top:0; color:#333;">Infirmary History</h3>
                ...
                <div id="sms-section" class="card admin-section hidden" style="border-top: 5px solid var(--info);">
                    ...
                    <div class="grid-2">
                        <div style="background: #f8f9fa; padding: 20px; border-radius: 12px; border: 1px solid #eee;">
                            ...
                            <select id="smsAudience" style="font-size:1.05rem; padding:15px;"><option value="all">Broadcast to ALL Active Parents</option><option value="arrears">Only Parents with Unpaid Tuition Arrears</option><option value="boarding">Only Parents of Boarding Students</option></select>
                        </div>
                        <div style="background: #e9ecef; padding: 20px; border-radius: 12px; border: 1px solid #dee2e6;">
                ...
                <div id="hr-section" class="card grid-2 admin-section hidden" style="border-top: 5px solid #6c757d;">
                    <div>
                        <div style="background: #f8f9fa; padding: 20px; border-radius: 12px; margin-bottom: 20px; border: 1px solid #eee;">
                            ...
                            <div class="grid-2"><input type="email" id="tEmail" placeholder="Teacher Email (Login ID)"><input type="password" id="tPass" placeholder="Temporary Password"></div>
                            <div class="grid-2"><input type="text" id="tPhone" placeholder="Mobile Number"><input type="text" id="tSubj" placeholder="Primary Assigned Subject"></div>
                        ...
                        <div style="background: #e9ecef; padding: 20px; border-radius: 12px; border: 1px solid #dee2e6;">
                            ...
                            <div class="grid-2"><input type="email" id="gEmail" placeholder="Parent Email Address"><input type="password" id="gPass" placeholder="Secure Password"></div>
                    ...
                    <div>
                        <div style="background: white; padding: 20px; border-radius: 12px; border: 1px solid #eee; height: 100%; box-sizing: border-box;">
                ...
                <div id="attendance-section" class="card admin-section {% if current_user.role == 'admin' %}hidden{% endif %}" style="border-top: 5px solid var(--info);">
                    ...
                    <div style="background: #e9ecef; padding: 25px; border-radius: 12px; max-width: 800px; border: 1px solid #dee2e6;">
                        <div class="grid-2" style="align-items: end;">
                            <div><label style="font-weight:bold; font-size:0.9rem; color:#555; display:block; margin-bottom:5px;">Select Date</label><input type="date" id="attDate" value="" style="margin-bottom:0;"></div>
                            <div><label style="font-weight:bold; font-size:0.9rem; color:#555; display:block; margin-bottom:5px;">Target Student ID</label><input type="number" id="attStuId" placeholder="ID" style="margin-bottom:0;"></div>
                        </div>
                        <div class="grid-2" style="margin-top: 20px;">
                            <select id="attStatus" style="margin-bottom:0; font-weight:bold;"><option value="Present">✅ Present (Include in Daily Feeding)</option><option value="Absent">❌ Absent (Remove from Daily Feeding)</option></select>
                            <button class="btn btn-info" style="margin-bottom:0;" onclick="sendAction('/api/attendance', {student_id: document.getElementById('attStuId').value, record_date: document.getElementById('attDate').value, status: document.getElementById('attStatus').value})">Commit Attendance</button>
                        </div>
                    </div>
                </div>
                ...
                <div id="academics-section" class="card grid-2 admin-section hidden" style="border-top: 5px solid var(--accent);">
                    <div>
                        <div style="background: #f8f9fa; padding: 20px; border-radius: 12px; border: 1px solid #eee;">
                            ...
                            <div class="grid-2"><input type="number" id="gStuId" placeholder="Student ID (Required)"><input type="text" id="gSub" placeholder="Subject Name"></div>
                            <div class="grid-2"><input type="number" id="gClass" placeholder="Class Score (30%)"><input type="number" id="gExam" placeholder="Exam Score (70%)"></div>
                            <div class="grid-2"><input type="text" id="gTerm" placeholder="Term (e.g. Term 1)"><input type="text" id="gYear" placeholder="Year (e.g. 2026)"></div>
                            <input type="text" id="gRem" placeholder="Teacher's Qualitative Remark">
                            <button class="btn btn-success" onclick="sendAction('/api/grades', {student_id: document.getElementById('gStuId').value, subject_name: document.getElementById('gSub').value, class_score: document.getElementById('gClass').value, exam_score: document.getElementById('gExam').value, term: document.getElementById('gTerm').value, academic_year: document.getElementById('gYear').value, remarks: document.getElementById('gRem').value})">Save Academic Record</button>
                        </div>
                    </div>
                    <div>
                        <div style="background: #e9ecef; padding: 20px; border-radius: 12px; border: 1px solid #dee2e6; height: 100%; box-sizing: border-box;">
                            ...
                            <input type="number" id="repId" placeholder="Enter Student ID">
                ...
                <div id="lesson-section" class="card hidden" style="border-top: 5px solid #ffc107;">
                    <h3>📚 GES Lesson Plan Inspection Vault</h3>
                    {% if current_user.role == 'teacher' %}
                    <div style="background: #f8f9fa; padding: 20px; border-radius: 12px; border: 1px solid #eee; margin-bottom: 20px;">
                        <div class="grid-2"><input type="text" id="lpSubj" placeholder="Subject"><input type="text" id="lpClass" placeholder="Class"></div>
                        <div class="grid-2"><input type="number" id="lpWeek" placeholder="Week Number"><input type="text" id="lpTopic" placeholder="Topic"></div>
                        <textarea id="lpContent" placeholder="Paste lesson plan notes here..." style="height: 100px; resize:none;"></textarea>
                        <button class="btn btn-success" onclick="sendAction('/api/lesson_plans', {subject: document.getElementById('lpSubj').value, class_name: document.getElementById('lpClass').value, week_number: document.getElementById('lpWeek').value, topic: document.getElementById('lpTopic').value, plan_content: document.getElementById('lpContent').value})">Submit to Headmaster</button>
                    </div>
                    {% endif %}
                ...
                <div id="cbt-section" class="card hidden" style="border-top: 5px solid #17a2b8;">
                    <h3>💻 Computer-Based Testing (CBT) Engine</h3>
                    <div style="background:#f8f9fa; padding:15px; border-radius:8px; margin-bottom:20px;">
                        <h4>Publish New Multiple-Choice Test</h4>
                        <div class="grid-2"><input type="text" id="cbtSubject" placeholder="Subject"><input type="text" id="cbtClass" placeholder="Class"></div>
                        <input type="text" id="cbtTitle" placeholder="Assessment Title (e.g. Midterm Objective Quiz)">
                        <div class="grid-2"><input type="text" id="cbtYear" placeholder="Academic Year (e.g. 2026)"><input type="text" id="cbtTerm" placeholder="Term"></div>
                        <textarea id="cbtQText" placeholder="Sample Question: What is the capital of Ghana?"></textarea>
                        <div class="grid-2"><input type="text" id="cbtOptA" placeholder="Option A"><input type="text" id="cbtOptB" placeholder="Option B"><input type="text" id="cbtOptC" placeholder="Option C"><input type="text" id="cbtOptD" placeholder="Option D"></div>
                        <input type="text" id="cbtCorrect" placeholder="Correct Option Letter (A, B, C, or D)">
                        <button class="btn btn-success" onclick="publishQuiz()">Publish Test & Synchronize to Grades</button>
                    </div>
                </div>
                ...
                <div id="calendar-section" class="card admin-section hidden" style="border-top: 5px solid #20c997;">
                    ...
                    <div class="grid-2">
                        ...
                        <div style="background: #f8f9fa; padding: 20px; border-radius: 12px; border: 1px solid #eee;">
                            ...
                            <input type="text" id="calTitle" placeholder="Event Title (e.g. Mid-Term Break)"><input type="date" id="calDate"><input type="text" id="calDesc" placeholder="Description">
            ...
            <div id="settings-section" class="card admin-section hidden" style="border-top: 5px solid #333;">
                ...
                <div style="background: #f8f9fa; padding: 25px; border-radius: 12px; border: 1px solid #eee; max-width: 600px;">
                    <label style="font-weight:bold; color:var(--primary); margin-bottom:8px; display:block;">Official School Address</label>
                    ...
                    <button class="btn btn-success" style="margin-top: 15px;" onclick="sendAction('/api/settings', {address: document.getElementById('setAddress').value, phone: document.getElementById('setPhone').value})">Save Profile Updates</button>
                </div>
                {% endif %}
            </div>
            {% endif %}

            <!-- Shared Universal Data Viewer -->
            {% if current_user.is_authenticated %}
        ...
        <script>
            ...
            function showSection(sectionId) {
                const sections = ['analytics-section', 'finance-section', 'admissions-section', 'attendance-section', 'academics-section', 'sms-section', 'hr-section', 'guardian-section', 'settings-section', 'sa-settings-section', 'transport-section', 'store-section', 'exeat-section', 'sickbay-section', 'calendar-section', 'lesson-section', 'cbt-section'];
                sections.forEach(id => {
            ...
            async function adminResetPassword() {
                ...
                if(!confirm(`FORCE RESET the password for ${email}?`)) return;
                try {
            ...
            async function saResetAdmin() {
                ...
                if(!email || !newP) { showToast("Provide Target Email and New Password.", true); return; }
                if(!confirm(`FORCE RESET the admin password for ${email}?`)) return;
                try {
            ...
            async function editStudent() {
                ...
                const payload = {
                    ...
                    guardian_contact: prompt("Update Guardian Phone Number:"), house: prompt("Update House Assignment:"), transport_route: prompt("Update Transport Route:")
                };
                try {
            ...
            async function bulkEnrollCSV() {
                ...
                if (!fileInput.files.length) { showToast("Select a CSV file.", true); return; }
                const formData = new FormData(); formData.append('file', fileInput.files[0]); showToast("Uploading roster...", false);
                try {
            ...
            async function deleteStudent(id) {
                if(!confirm("WARNING: This will permanently delete the student and ALL associated grades, fees, and records. Continue?")) return;
                try {
            ...
            async function deleteBill(feeId, stuId) {
                if(!confirm("Reverse this bill and erase its record?")) return;
                try {
            ...
            async function registerStaff() {
                const payload = { email: document.getElementById('tEmail').value, password: document.getElementById('tPass').value, phone: document.getElementById('tPhone').value, subject: document.getElementById('tSubj').value, salary: document.getElementById('tSal').value };
                try {
                    const res = await fetch('/api/register_staff', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(payload) });
                    const data = await res.json();
                    if(res.ok) { showToast(data.message); document.getElementById('tEmail').value = ''; document.getElementById('tPass').value = ''; loadStaff(); } 
                    else { showToast(data.error, true); }
                } catch(e) { showToast("Connection failed", true); }
            }
            ...
            async function registerGuardian() {
                ...
                try {
                    ...
                    const data = await res.json();
                    if(res.ok) { showToast(data.message); document.getElementById('gEmail').value = ''; document.getElementById('gPass').value = ''; } 
                    else { showToast(data.error, true); }
                } catch(e) { showToast("Connection failed", true); }
            }
            ...
            async function issueBill() {
                ...
                const payload = { student_id: stuId, fee_category: document.getElementById('bCat').value, amount_due: document.getElementById('bAmount').value, academic_year: document.getElementById('bYear').value, term: document.getElementById('bTerm').value, description: document.getElementById('bDesc').value };
                try {
            ...
            async function processPayment(feeId, balance, stuId) {
                ...
                const amount = prompt("Payment amount (GHS) for Fee ID " + feeId + ". Outstanding: GHS " + balance);
                if(!amount || isNaN(amount) || amount <= 0) return;
                const method = prompt("Payment method (Cash / MoMo):", "Cash");
                if(!method) return;
                try {
            ...
            async function loadLessonPlans() {
                ...
                let html = '<table style="width:100%; border-collapse:collapse;"><tr><th style="padding:10px; border-bottom:2px solid #ddd; text-align:left;">Teacher</th><th style="padding:10px; border-bottom:2px solid #ddd; text-align:left;">Subject</th><th style="padding:10px; border-bottom:2px solid #ddd; text-align:left;">Topic</th><th style="padding:10px; border-bottom:2px solid #ddd; text-align:left;">Status</th><th style="padding:10px; border-bottom:2px solid #ddd; text-align:left;">Action</th></tr>';
                d.data.forEach(p => { html += `<tr><td style="padding:10px; border-bottom:1px solid #eee;">${p.teacher_email}</td><td style="padding:10px; border-bottom:1px solid #eee;">${p.subject}</td><td style="padding:10px; border-bottom:1px solid #eee;">${p.topic}</td><td style="padding:10px; border-bottom:1px solid #eee;"><b>${p.status}</b></td><td style="padding:10px; border-bottom:1px solid #eee;"><button class="btn btn-success" style="padding:6px 12px; margin:0;" onclick="reviewPlan(${p.plan_id}, 'Approved')">Approve</button></td></tr>`; });
