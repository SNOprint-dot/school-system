...
def superadmin_reset_admin():
    ...
    try:
        ...
        if not cur.fetchone(): return jsonify({"error": "Admin email not found."}), 404
...
# --- OPERATIONAL MODULES (TRANSPORT, INVENTORY, EXEAT, SICKBAY, CALENDAR) ---
...
def manage_exeats():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            conn.commit(); return jsonify({"message": "Exeat issued successfully."}), 201
        except Exception as e: conn.rollback(); return jsonify({"error": str(e)}), 500
        finally: cur.close(); conn.close()
...
# --- TENANT ENDPOINTS (STUDENTS, GRADES, BILLS, EXPENSES) ---
...
def bill_student():
    ...
    try:
        ...
        cur.execute("INSERT INTO audit_logs (school_id, user_email, action, target) VALUES (%s, %s, %s, %s)", (current_user.school_id, current_user.email, 'Issued Bill', f"GHS {d.get('amount_due')} to ID {d.get('student_id')}"))
...
def log_payment():
    d = request.get_json(); conn = get_db_connection(); cur = conn.cursor()
    try:
        cur.execute("INSERT INTO payments (fee_id, amount_paid, payment_method) VALUES (%s, %s, %s)", (int(d.get('fee_id')), float(d.get('amount_paid')), d.get('payment_method')))
        cur.execute("INSERT INTO audit_logs (school_id, user_email, action, target) VALUES (%s, %s, %s, %s)", (current_user.school_id, current_user.email, 'Logged Payment', f"GHS {d.get('amount_paid')} (Fee {d.get('fee_id')})"))
...
def add_grade():
    d = request.get_json(); conn = get_db_connection(); cur = conn.cursor()
    try:
        c_score = int(str(d.get('class_score') or '0').strip())
        e_score = int(str(d.get('exam_score') or '0').strip())
...
def register_staff():
    ...
    try:
        ...
        conn.commit(); return jsonify({"message": "Staff Profile Created!"}), 201
...
# --- 8. THE FULL MONOLITHIC ENTERPRISE FRONTEND DASHBOARD ---
@app.route('/dashboard')
def dashboard():
    ...
    <body>
        ...
        <main class="main-content">
            ...
            {% elif current_user.role in ['admin', 'teacher'] %}
                ...
                <div id="exeat-section" class="card grid-2 admin-section hidden" style="border-top: 5px solid #e83e8c;">
                    <div>
                        <div style="background: #f8f9fa; padding: 20px; border-radius: 12px; border: 1px solid #eee;">
                            ...
                            <button class="btn" style="background:#e83e8c;" onclick="sendAction('/api/exeats', {student_id: document.getElementById('exStuId').value, exeat_type: document.getElementById('exType').value, reason: document.getElementById('exReason').value, expected_return: document.getElementById('exReturn').value})">Issue Exeat</button>
                        </div>
                    </div>
                    <div>
                ...
                <!-- Shared Academic Calendar Section -->
        ...
        <script>
            ...
            async function deleteStudent(id) {
                if(!confirm("Delete this student permanently? All linked grades, bills, and records will be dropped.")) return;
                try {
            ...
            async function issueBill() {
                ...
                try {
                    ...
                    const data = await res.json();
                    if(res.ok) { showToast(data.message); document.getElementById('bAmount').value = ''; loadStatement(stuId); } 
                    else { showToast(data.error, true); }
                } catch(e) { showToast("Connection failed", true); }
            }
                ...
                rows.forEach(row => {
                    ...
                    keys.forEach(k => {
                        ...
                        if (k === 'photo') { html += `<td style="padding:15px; border-bottom:1px solid #eee; width: 60px;"><img src="/api/photo/${row['student_id']}" style="width:45px; height:45px; border-radius:8px; object-fit:cover; border:2px solid #ccc; background:#eee;"></td>`; } 
                            ...
                            html += `<button class="btn btn-danger" style="padding:6px 12px; font-size:0.85rem; margin:0 0 0 5px; width:auto; background:white; color:var(--danger); border:1px solid var(--danger);" onclick="deleteBill(${row['fee_id']}, ${row['student_id']})">Reverse Bill</button></td>`;
                        } 
